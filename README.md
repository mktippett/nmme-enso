...
| `roni_factor_backout.py` | Exploratory/diagnostic (no production figures): backs out NOAA CPC's Relative ONI (RONI) per-calendar-month scaling factor from ERSSTv6 + CPC's published `Rnino34.ascii.txt` (monthly) and `RONI.ascii.txt` (seasonal), fit directly per calendar month by OLS — unrelated to this project's own n34r index; reproduces an external CPC product for comparison. Written to `plots/roni_factor_backout/`; see `docs/CPC_RONI_scaling.md` for the write-up | none. See `specs/roni_factor_backout.md`. |
