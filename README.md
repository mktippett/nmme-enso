...
| `roni_factor_backout.py` | Exploratory/diagnostic (no production figures): backs out NOAA CPC's Relative ONI (RONI) per-calendar-month scaling factor from ERSSTv6 + CPC's published `RONI.ascii.txt`, via a joint 12-month linear fit (CPC scales monthly relative-index values, then averages 3 months into a season) — unrelated to this project's own n34r index; reproduces an external CPC product for comparison. Written to `plots/roni_factor_backout/` | none. See `specs/roni_factor_backout.md`. |
...
| CPC Relative ONI (RONI) table | [NOAA CPC](https://www.cpc.ncep.noaa.gov/data/indices/RONI.ascii.txt) | Used only by `roni_factor_backout.py`. No refresh command yet — manual re-download. |
