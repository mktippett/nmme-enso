...
| `roni_factor_backout.py` | Exploratory/diagnostic (no production figures): backs out NOAA CPC's Relative ONI (RONI) per-calendar-month scaling factor from ERSSTv6 + CPC's published `Rnino34.ascii.txt` (monthly) and `RONI.ascii.txt` (seasonal), fit directly per calendar month by OLS, and identifies CPC's publication convention (values floored to 2 decimals, not rounded) — unrelated to this project's own n34r index; reproduces an external CPC product for comparison. Written to `plots/roni_factor_backout/`; see `docs/CPC_RONI_scaling.md` for the write-up | none. See `specs/roni_factor_backout.md`. |
...
| ERSSTv6 monthly SST | [NOAA PSL](https://downloads.psl.noaa.gov/Datasets/noaa.ersst.v6/sst.mnmean.nc) | Used only by `roni_factor_backout.py`, since CPC's RONI is explicitly ERSSTv6-based. No refresh command yet — manual re-download. |
| CPC Relative ONI (RONI) table, seasonal | [NOAA CPC](https://www.cpc.ncep.noaa.gov/data/indices/RONI.ascii.txt) | Used only by `roni_factor_backout.py`. No refresh command yet — manual re-download. |
| CPC relative Niño-3.4 table, monthly | [NOAA CPC](https://www.cpc.ncep.noaa.gov/data/indices/Rnino34.ascii.txt) | Used only by `roni_factor_backout.py`. No refresh command yet — manual re-download. |
